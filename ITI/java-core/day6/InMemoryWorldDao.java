...
public class InMemoryWorldDao implements WorldDao {
	...
	public static void main(String args[]) {
		...
		*/
		...
		//allCountries.values().stream().map(C -> C.getCities()).map(citty -> citty.stream().map( c -> c.getPop()).collect(toList())   ).collect(toList());
		//Map<String,list<City>> country_city =  allCountries.values().stream().map(c -> c.getCities()).map(citty -> citty.stream()).collect(Collectors.groupingBy(City::getCountryCode)  );
		//List<Integer> cities = allCountries.values().stream().flatMap(c -> (c.getCities()).stream() ).map(cc -> cc.getPop()).collect(toList());
		
		List<Country> allCountry = worldDao.findAllCountries();
		
		///////////////////////////////////////////////////////////////////
		System.out.println("Question 1:");
		List<City> cities = allCountry.stream().map(C -> C.getCities().stream()
									.max(Comparator.comparing(City::getPop))) //gets class city that has max population
									.filter(Optional::isPresent) //returns true if value not null
									.map(Optional::get) //gets the value
									.collect(toList());
		cities.stream().forEach(value -> System.out.println("Country: " + worldDao.findCountryByCode(value.getCountryCode()).getName() +
									"  city: "+ value.getName() + 
									"  population: "+ value.getPop()));
		/////////////////////////////////////////////////////////////////////////////////
		System.out.println();
		System.out.println("Question 2:");
		/*List<City> cities2 = worldDao.getAllContinents().stream().map( c -> (worldDao.findCountriesByContinent(c)
												.stream()
												.FlatMap(countryy -> countryy.getCities().stream())
		...
		//List<List<City>> countries2 = worldDao.getAllContinents().stream().map( c -> worldDao.findCountriesByContinent(c).stream().flatMap(countryy ->countryy.getCities().stream()).collect(toList())  ).collect(toList());
		
		List<City> cities2 = worldDao.getAllContinents().stream()
					.map( 
						c -> worldDao.findCountriesByContinent(c)
							     .stream()
							     .flatMap(countryy ->countryy.getCities().stream())
							     .max(Comparator.comparing(City::getPop))
					    )
		...
		cities2.stream().forEach(value -> System.out.println("Continent: " + worldDao.findCountryByCode(value.getCountryCode()).getContinent() +
									"  city: "+ value.getName() +
									 "  population: "+ value.getPop() ) );
		...
		City city3 = allCountry.stream().map(c -> worldDao.findCityById(c.getCapital())) 
						.filter(cityy -> cityy != null)
						.max(Comparator.comparing(City::getPop)).get();
						//.ifPresent(System.out::println);

		
		System.out.println("Continent: " + worldDao.findCountryByCode(city3.getCountryCode()).getContinent()+ 
				"  Country: " + worldDao.findCountryByCode(city3.getCountryCode()).getName() +
				"  city: "+ city3.getName() + 
				"  population: "+ city3.getPop() );
